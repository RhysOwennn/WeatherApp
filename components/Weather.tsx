...
const Weather = ({
    ...
    <div>
      ...
      <div className='flex flex-row border-2 border pl-2'>
        Date: {weather.dt_txt} - {weather.weather[0].description} {" "}
        ...
        <Image src={`https://openweathermap.org/img/wn/${data.list?.[index].weather[0].icon}@2x.png`} alt={'/'}  width='100' height='100'/>
      </div>))}</div>
    </div>
  )
